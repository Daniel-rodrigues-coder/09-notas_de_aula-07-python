...
## Parte 1 — Estruturas de Repetição com `while`

> Nos exercícios abaixo, utilize **apenas** a instrução `while` para controlar a repetição.
...
**14.** Escreva um programa que recebe um número inteiro `base` e um número inteiro positivo `expoente`, e calcula `base ** expoente` **sem usar o operador `**`** — use apenas multiplicações repetidas com `while`.
...
## Parte 2 — Estruturas de Repetição e Seleção com `while` + `if`

> Nos exercícios abaixo, utilize a instrução `while` para controlar a repetição e `if` / `elif` / `else` para as decisões.
...
**34.** Escreva um programa que recebe dois inteiros positivos `a` e `b` e calcula o Máximo Divisor Comum (MDC) usando o algoritmo de Euclides com `while`.
